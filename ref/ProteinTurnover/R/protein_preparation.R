...
readScaffoldFile <- function(file, times, dir, verbose=TRUE) {
    ...
    r <- r[,c(
              ...
              "Sequence",
              "RT_min",            
              "Mono_Mass", ## Updated 29 May 2020 from "Peptide_Mass"
              "Charge",
              "TimePoint",
              "Probability"
              )]
...
makeSequences <- function(d, isotope, confidence.level, time.zero, time.unit,
                          ...
                          aa.table=mod_aaTable, verbose=TRUE) {
    if(verbose) message("finding Sequences ...")       
    ds <- split(d, paste(d$ID, d$Sequence, sep="--")) ## split table into list of smaller tables by ID--Sequence
...
addEICs <- function(dat, files, type=c("mzXML", "rds", "obj"),
    ...
    stopifnot(length(files)==length(times))
    type <- match.arg(type)   
    for(i in seq_along(files)) { ## i == 1:number files
        time <- times[i]
        if(type=="mzXML") {
...
getCount <- function(robj, RT, MW, Z, nchannels, isotope, rt.tol, mz.tol, 
    ...
    H_WEIGHT <- 1.007277
    #mz <- ((MW + (0:(nchannels-1))*delta[[isotope]]) / Z) + H_WEIGHT ## original
    mz <- ((MW + (Z*H_WEIGHT)) + (0:(nchannels-1))*delta[[isotope]]) / Z ## should be
    rtr <- RT + rt.tol * c(-1, 1)
    count <- tryCatch({
        ## get first MZ to get RT and to get matrix size
        #mzr <- mz[1] + mz.tol * c(-1, 1)
        mzr <- mz[1] + (mz[1] * mz.tol * c(-1, 1)) ## Modified 29 May 2020 to be proportional
        ...
        for(i in 2:length(mz)) {
            ...
            mzr <- mz[i] + (mz[i] * mz.tol * c(-1, 1)) ## Modified 29 May 2020 to be proportional
...
addEIC <- function(x, robj, time, mz.tol=x$pars$mz.tol, rt.tol=x$pars$rt.tol,
    ...
    tot <- sapply(counts, function(x) sum(x$Count)) ## sums the entire intensity matrix
    x$Times$Total[k] <- tot
...
sequenceElementCount <- function(sequence, isotope, aa.table=mod_aaTable) {
    ions <- c("C","H","N","O","S")
