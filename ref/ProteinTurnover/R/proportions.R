...
#'    output data frames.
#' @param method Options are "lm", "rlm", "lqs", "rq", "sum", and "log". ### "tot" added 08May2020 AEZ
...
relAbFromCounts <- function(Count, Channel, RT, data, TimePoint,
                            method=c("lm","rlm", "lqs", "rq", "sum","log", "tot"),
                            norm_channel,
                            threshold,
                            cutoff,
                            nboot=0) {
  ...
  ## Split df into list by channel; each element is a df of Count and RT
  d.wide <- split(d.long[,c("Count","RT")], d.long$Channel)
  ## Transform list into matrix: channels = columns, unique RT = rows, values = counts
...
## d.wide
  ...
  ## Set norm_channel to always be the monoisotopic peak (AEZ)
  ## Reversed 30 Apr 2020; deleted: norm_channel <- c("0" = as.integer(1))
  ...
  ## Declare new Rsq object with one NA value for each channel in the data
  ...
  if(method %in% c("lm","rlm", "lqs", "rq")) {
    ...
    Rsq <- matrix(nrow=length(channels), ncol=1, dimnames=list(names(channels), c("R_squared")))
    if(nboot>0) {
    ...
    for(idx in channels) {
      if(norm_channel==idx) {
        ...
        ## x and y are named numeric vectors (names = RTs, values = counts)
        ## x = base/monoisotopic peak, y = each consecutive channel
        x <- d.wide[,norm_channel]
        y <- d.wide[,idx]
        ok <- !is.na(x) & !is.na(y)
        ## Check that there are at least 2 RTs to compare across channels
        if(sum(ok) >=2) {
          x <- x[ok]
          y <- y[ok]
          ...
          Rsq[idx,] <- summary(m)$r.squared ## changed from Rsq[idx]
        } else {
    ...
    relab.rsq <- as.numeric(Rsq[,1]) ## added by AEZ 2020-04-09
    relab <- relab.obs
    relab[is.na(relab)] <- 0
    relab[relab<0] <- 0
    out.class <- "regRelAb"
  } else if(method=="sum") {
    se <- NULL ## added by AEZ 2020-04-30
    Rsq <- NULL ## added by AEZ 2020-04-30
    boot <- NULL ## added by AEZ 2020-04-30
    relab <- as.numeric(colSums(d.wide, na.rm=TRUE)) ## sum counts for each channel
    relab <- relab/sum(relab) ## make into proportion of total counts across all channels
    relab <- relab/relab[norm_channel] ## normalize the proportion counts in norm_channel
    relab[is.na(relab)] <- 0
    coefs <- cbind(intercept=0, slope=relab)
    rownames(coefs) <- colnames(d.wide)
    relab.obs <- as.numeric(coefs[,2]) ## added by AEZ 2020-04-29
    relab.rsq <- NA ## added by AEZ 2020-04-29
    relab <- relab.obs ## added by AEZ 2020-04-29
    out.class <- "regRelAb"
    } else if(method=="tot") {
    se <- NULL ## added by AEZ 2020-04-30
    Rsq <- NULL ## added by AEZ 2020-04-30
  ...
  #  relab <- relab/relab[norm_channel] ## normalize the proportion counts in norm_channel
    ...
    coefs <- cbind(intercept=0, slope=relab)
    rownames(coefs) <- colnames(d.wide)
    relab <- as.numeric(coefs[,2]) ## added by AEZ 2020-04-29 ## changed from 'relab.obs' to 'relab' 2020-05-15
    relab.obs <- as.numeric(colSums(d.wide, na.rm=TRUE)) ## added by AEZ 2020-04-29 ## changed from 'relab <- relab.obs' 2020-05-15
    relab.rsq <- sum(relab) ## added by AEZ 2020-04-29 ## changed from NA 2020-05-15
    out.class <- "regRelAb"
  } else if(method=="log") {
    d.long$logCount <- log(d.long$Count) ## add column of log transformed count
    dx <- within(d.long[d.long$use>0,], { ## make new data frame
      RT <- factor(RT)
      Channel <- factor(Channel)
    })
 #   dx[dx == -Inf] <- NA ## added by AEZ 2020-04-30
    m <- stats::lm(logCount ~ RT + Channel, data=dx, na.action = na.exclude)
    coefs <- stats::coef(m)
  ...
  out <- data.frame(Channel=as.numeric(colnames(d.wide)),
                    ...
                    RelAb.obs=relab.obs,
                    RelAb.se=relab.se,
                    RelAb.Rsq=relab.rsq,  ## added by AEZ 2020-04-09
